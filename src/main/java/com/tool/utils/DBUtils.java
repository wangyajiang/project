...
public class DBUtils {
	...
	public static <T> List<T> queryAllByEntity(DBHelper db, Class<?> clazz) throws DaoException {
		try {
			String entityName = ConvertUtils.lowerFirstName(clazz.getSimpleName());
			String tabName = ConvertUtils.camelToUnderline(entityName);
			StringBuilder sql = new StringBuilder("select * from ");
			sql.append(tabName);
			return db.queryList(clazz, sql.toString(), null);
		} catch (Exception e) {
			e.printStackTrace();
			throw new DaoException();
		}
	...
	public static List<Map<String, Object>> queryAllByMap(DBHelper db, String tableName) throws DaoException {
		try {
			StringBuilder sql = new StringBuilder("select * from ");
			sql.append(tableName);
			return db.queryList(Map.class, sql.toString(), null);
		} catch (Exception e) {
			e.printStackTrace();
			throw new DaoException();
		}
